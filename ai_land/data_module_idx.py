...
class EcDataset(Dataset):
    ...
    def __init__(
        ...
        if "clim_glm" in self.static_feat_lst:
            ...
            glm = self.ds_ecland.clim_data[
                :, list(self.ds_ecland["clim_variable"]).index("clim_glm")
            ]
            glm_arr = torch.zeros(glm.shape)
            glm_arr[np.where(glm > 0.8)] = 1
            glm_arr = glm_arr.reshape(1, self.x_size, -1)
    ...
    def __len__(self):
        return (self.len_dataset - 1 - self.rollout) * self.x_size

    # get a row at an index
    def __getitem__(self, idx):
        t_idx = (idx % (self.len_dataset - 1 - self.rollout)) + self.start_index
        t_idx2 = (
        ...
        # print(f"roll: {self.rollout}, lends: {self.len_dataset}, st_idx: {self.start_index}")
        # print(f"idx: {idx}, t_idx: {t_idx}, t_idx2: {t_idx2}, {t_idx2 - t_idx}")
        x_idx = (idx % self.x_size) + self.x_idxs[0]
        # print(f"x_idx: {x_idx}")

        ds_slice = tensor(self.ds_ecland.data[slice(t_idx, t_idx2), x_idx, :])
        # print(f"ds: {ds_slice.shape}")

        X = ds_slice[:, self.dynamic_index]
        X = self.transform(X, self.x_dynamic_means, self.x_dynamic_stdevs)
        # print(f"X: {X.shape}")

        X_static = self.x_static_scaled.expand(self.rollout, -1, -1)[:, x_idx, :]
        # print(f"Stat: {X_static.shape}")

        Y_prog = ds_slice[:, self.targ_index]
        Y_prog = self.transform(Y_prog, self.y_prog_means, self.y_prog_stdevs)
        # print(f"prog: {Y_prog.shape}")
